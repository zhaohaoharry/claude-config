...
modified: 2026-08-27T08:56:55.737Z
---
...
**Broken again on 2026-08-27 (manuscript_v4.pdf):** I appended `Start-Process code -ArgumentList <pdf>` to every recompile command. Each call re-opened the tab in the active editor group and collapsed the user's split review panels ("the pdf used to refresh only, why it reopens now? that causes the division of review panels to combine every time"). Rule restated: in an edit-compile loop on a manuscript the user already has open, the compile command ends at pdflatex. No open call of any kind, ever, on a recompile.
